...
function generateCompactPNG(agencias: Agencia[], query: string): string {
  ...
  let svg = `
    <svg width="${width}" height="${totalHeight}" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- Header Content -->
      <text x="${padding}" y="35" class="header">Shalom - Búsqueda por Nombre</text>
      <text x="${padding}" y="58" class="subtitle">Término: "${escapeXml(query)}" - ${agencias.length} resultado(s)</text>
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Filtrar agencias SOLO por nombre y excluir las que tienen ter_habilitado_OS = 0
    ...
    console.error('Error en API de imagen-nombre:', error);
    return NextResponse.json(
