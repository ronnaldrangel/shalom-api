...
export async function GET(request: NextRequest) {
  ...
  const authResult = await authMiddleware(request, '/api/nombre');
  ...
  try {
    ...
    const resultados = agencias.filter((agencia: Agencia) => {
      ...
      if (agencia.ter_habilitado_OS === 0) {
        return false;
      }
      
      // Buscar solo en el campo nombre
    ...
    console.error('Error en /api/nombre:', error);
    return NextResponse.json(
